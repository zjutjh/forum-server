...
public interface AnnouncementMapper extends BaseMapper<Announcement> {
         ...
         * 查询已发布状态（status=1）或定时发布时间已到（status=2且scheduled_at<=NOW()）的公告
         * 
        ...
        @Select("<script>" +
                        ...
                        "WHERE deleted = false " +
                        "AND (status = 1 OR (status = 2 AND scheduled_at &lt;= NOW())) " +
        ...
        /**
         * 统计用户可见的公告总数（支持类型筛选）
         * 统计已发布状态（status=1）或定时发布时间已到（status=2且scheduled_at<=NOW()）的公告
         * 
         * @param type 公告类型（可选筛选）
         * @return 符合条件的公告总数
         */
        ...
        @Select("<script>" +
                        ...
                        "LIMIT #{offset}, #{limit}" +
                        "</script>")
        List<Announcement> findScheduledAnnouncements(
                    @Param("orderType") Integer orderType,
                    @Param("offset") Integer offset,
                    @Param("limit") Integer limit);
        ...
        /**
         * 查询已发布的公告并支持升序/降序排序（管理员版，支持查询已删除的）
         */
        @Select("<script>" +
                        "SELECT * FROM announcement " +
                        "WHERE status = 1 " +
        ...
        /**
         * 查询待发布的公告并支持升序/降序排序（管理员版，支持查询已删除的）
         */
        @Select("<script>" +
                        "SELECT * FROM announcement " +
                        "WHERE status = 2 " +
        ...
        /**
         * 查询草稿公告并支持升序/降序排序（管理员版，支持查询已删除的）
         */
        @Select("<script>" +
                        "SELECT * FROM announcement " +
                        "WHERE status = 0 " +
